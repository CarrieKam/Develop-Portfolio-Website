        ...
        {achievements.map((achievement, index) => (
          <div key={index} className="relative">
            ...
            {achievement.title === 'Hackathon' && (
              <div className="absolute left-8 top-full w-px h-24 bg-[#3DB7CA] p-2 z-10" />
            )}
            
            <div
              className={`p-6 rounded-lg transition-colors duration-200 relative ${
                achievement.title === 'McHacks9 Entertainer Prize'
                  ? 'ml-16 bg-[#3DB7CA] dark:bg-[#1a2c34]'
                  : 'bg-[#3DB7CA] dark:bg-[#1a2c34]'
              }`}
            >
              {/* Connection line for McHacks9 */}
              {achievement.title === 'McHacks9 Entertainer Prize' && (
                <div className="absolute -left-8 top-1/2 w-8 bg-[#3DB7CA] p-2" />
              )}
